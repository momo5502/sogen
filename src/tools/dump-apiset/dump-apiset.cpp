...
namespace sogen
{
    void print_apiset(sogen::PAPI_SET_NAMESPACE api_set_map)
    {
        for (ULONG i = 0; i < api_set_map->Count; i++)
        {
            auto* const entry = reinterpret_cast<sogen::PAPI_SET_NAMESPACE_ENTRY>(
                reinterpret_cast<ULONG_PTR>(api_set_map) + api_set_map->EntryOffset + i * sizeof(sogen::API_SET_NAMESPACE_ENTRY));
            ...
            {
                auto* const value = reinterpret_cast<sogen::PAPI_SET_VALUE_ENTRY>(
                    reinterpret_cast<ULONG_PTR>(api_set_map) + entry->ValueOffset + x * sizeof(sogen::API_SET_VALUE_ENTRY));
                // printf("  Value %d\n", x);
                // printf("    Flags: %08X\n", value->Flags);
                // printf("    NameOffset: %08X\n", value->NameOffset);
                // printf("    NameLength: %08X\n", value->NameLength);
...
int main()
{
    ...
    auto* const peb = static_cast<sogen::PPEB64>(sogen::GetCurrentProcessPeb());
    auto* const api_set_map = reinterpret_cast<sogen::API_SET_NAMESPACE*>(peb->ApiSetMap);
    ...
    sogen::print_apiset(api_set_map);
    ...
    const auto compressed = sogen::utils::compression::zstd::compress(buffer);
    if (compressed.empty())
    {
    ...
    sogen::utils::io::write_file("api-set.bin", compressed, false);
