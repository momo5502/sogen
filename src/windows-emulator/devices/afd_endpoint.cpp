...
namespace
{
    ...
    std::vector<std::byte> convert_host_address_to_transport_address(windows_emulator& win_emu,
                                                                     const network::address& host_addr)
    {
        auto win_addr_bytes = convert_to_win_address(win_emu, host_addr);

        const auto win_sockaddr_len = win_addr_bytes.size() - 2;
        const auto* win_sockaddr = reinterpret_cast<const TDI_ADDRESS_IP*>(win_addr_bytes.data() + 2);

        const size_t ta_address_size = offsetof(TA_ADDRESS, Address) + win_sockaddr_len;
        const size_t transport_address_size = offsetof(TRANSPORT_ADDRESS, Address) + ta_address_size;

        std::vector<std::byte> transport_buffer(transport_address_size);

        auto* transport_addr = reinterpret_cast<TRANSPORT_ADDRESS*>(transport_buffer.data());
        // transport_addr->TAAddressCount = 1;

        auto* ta_addr = reinterpret_cast<TA_ADDRESS*>(transport_addr->Address);
        // ta_addr->AddressLength = static_cast<USHORT>(win_sockaddr_len);
        ta_addr->AddressType = 2; // TDI_ADDRESS_TYPE_IP

        memcpy(ta_addr->Address, win_sockaddr, win_sockaddr_len);
    ...
    struct afd_endpoint : io_device
    {
        ...
        NTSTATUS ioctl_listen(windows_emulator& win_emu, const io_device_context& c) const
        {
            ...
            if (c.input_buffer_length < sizeof(int))
            {
                return STATUS_BUFFER_TOO_SMALL;
            }

            const auto backlog = win_emu.emu().read_memory<int>(c.input_buffer);

            if (!this->s_->listen(backlog))
            {
                return STATUS_INVALID_PARAMETER; // TODO: Is this the correct status?
            }
        ...
        NTSTATUS ioctl_wait_for_listen(windows_emulator& win_emu, const io_device_context& c)
        {
            ...
            constexpr size_t ta_address_size = offsetof(TA_ADDRESS, Address) + sizeof(TDI_ADDRESS_IP);
            constexpr size_t total_response_size = offsetof(AFD_LISTEN_RESPONSE_INFO, RemoteAddress) +
                                                   offsetof(TRANSPORT_ADDRESS, Address) + ta_address_size;

            if (c.output_buffer_length < total_response_size)
            {
                return STATUS_BUFFER_TOO_SMALL;
            }
            ...
            if (!accepted_socket_ptr)
            {
                ...
                return STATUS_UNSUCCESSFUL; // TODO: Is this the correct status?
            }
            ...
            auto transport_buffer = convert_host_address_to_transport_address(win_emu, remote_address);

            memcpy(&response.RemoteAddress, transport_buffer.data(), offsetof(TRANSPORT_ADDRESS, Address));
            memcpy(response.RemoteAddress.Address, transport_buffer.data() + offsetof(TRANSPORT_ADDRESS, Address),
                   ta_address_size);

            win_emu.emu().write_memory(c.output_buffer, &response, total_response_size);

            if (c.io_status_block)
            {
                IO_STATUS_BLOCK<EmulatorTraits<Emu64>> block{};
                block.Status = STATUS_SUCCESS;
                block.Information = static_cast<ULONG_PTR>(total_response_size);
                c.io_status_block.write(block);
            }
