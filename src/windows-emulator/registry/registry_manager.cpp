...
namespace sogen
{
    ...
    // folder it expects, making the RDP audio device usable as the default playback/recording endpoint. The
    // hive reflects MMDevices under both the native path and Wow6432Node; 32-bit (WoW64) audio apps reach it
    // through the redirected Wow6432Node path and 64-bit apps through the native one, so alias both roots.
    void registry_manager::alias_remote_audio_endpoints(const std::filesystem::path& machine)
    {
        const auto current_version = machine / "SOFTWARE" / "Microsoft" / "Windows" / "CurrentVersion";
        const auto current_version_wow = machine / "SOFTWARE" / "Wow6432Node" / "Microsoft" / "Windows" / "CurrentVersion";

        for (const auto& audio : {current_version / "MMDevices" / "Audio", current_version_wow / "MMDevices" / "Audio"})
        {
            this->alias_remote_audio_folder(audio);
        }
    }

    void registry_manager::alias_remote_audio_folder(const std::filesystem::path& audio)
    {
