...
namespace syscalls
{
    namespace
    {
        ...
        // SystemModuleInformation reports loaded *kernel* modules. We don't
        // emulate the kernel, but a lot of code (anti-analysis, driver
        // enumeration, KASLR-base resolution) expects at least ntoskrnl.exe
        // at a plausible kernel address, so we fake a single entry.
                                                     ...
                                                     const emulator_object<uint32_t> return_length)
        {
            ...
            entry.NextOffset = 0; // single (and therefore last) entry
