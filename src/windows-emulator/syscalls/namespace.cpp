...
namespace syscalls
{
    ...
    static constexpr std::u16string get_description(const std::u16string_view name, const std::u16string_view boundary_name)
    {
    ...
    static bool check_sid(const SID* sid)
    {
    ...
    static NTSTATUS open_or_create_ns(const syscall_context& c, const emulator_object<handle> namespace_handle,
    ...
    NTSTATUS handle_NtDeletePrivateNamespace(const syscall_context& c, const handle namespace_handle)
    {
        ...
        c.win_emu.callbacks.on_generic_access("Deleting private namespace", get_description(ns->name, ns->boundary_name));
        ns->deleted = true;
