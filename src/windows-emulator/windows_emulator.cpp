...
namespace sogen
{
    ...
    namespace
    {
        // TEMPORARY: prints the byte-offset range of each top-level deserialize() step so a captured
        // mismatch dump can be bisected to the exact component that owns the divergence. Gated on an env
        // var so normal test runs are unaffected. Revert with the rest of the debug instrumentation.
