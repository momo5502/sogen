...
pe_detection_result pe_architecture_detector::detect_from_memory(uint64_t base_address, uint64_t image_size)
{
    ...
    if (std::holds_alternative<std::error_code>(variant_result)) {
        pe_detection_result result;
        result.error_message = "Failed to detect PE architecture from memory at 0x" + 
...
mapped_module pe32_mapping_strategy::map_from_memory(memory_manager& memory, uint64_t base_address, 
                                                    uint64_t image_size, const std::string& module_name)
{
...
mapped_module pe64_mapping_strategy::map_from_memory(memory_manager& memory, uint64_t base_address, 
                                                    uint64_t image_size, const std::string& module_name)
{
...
mapped_module* module_manager::map_module_core(const pe_detection_result& detection_result,
                                              std::function<mapped_module()> mapper,
...
{
    auto detection_result = pe_architecture_detector::detect_from_file(
        this->file_sys_->translate(executable_path));
...
void module_manager::load_native_64bit_modules(const windows_path& executable_path, 
                                              const windows_path& ntdll_path, 
                                              const windows_path& win32u_path, 
                                              const logger& logger)
{
...
void module_manager::load_wow64_modules(const windows_path& executable_path,
                                       const windows_path& ntdll_path,
                                       const windows_path& win32u_path,
                                       ...
                                       const logger& logger)
{
    ...
    init_block.Wow64SharedInformation[static_cast<uint64_t>(WOW64_SHARED_INFORMATION_V5::SharedNtdll32pQueryProcessDebugInformationRemote)] = 
...
void module_manager::map_main_modules(const windows_path& executable_path, const windows_path& system32_path, const windows_path& syswow64_path,
                                      const logger& logger)
{
...
mapped_module* module_manager::map_local_module(const std::filesystem::path& file, const logger& logger, const bool is_static)
{
    ...
    return map_module_core(detection_result, [&]() {
        auto& strategy = strategy_factory_.get_strategy(detection_result.architecture);
