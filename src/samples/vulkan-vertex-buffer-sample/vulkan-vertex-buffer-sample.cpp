// A windowed Vulkan guest app that draws a *vertex-buffer-driven* quad and reports its frame rate. It
// builds on vulkan-spinning-triangle-sample but, instead of baking the geometry into the vertex shader
// (gl_VertexIndex), it feeds real geometry through the GPU bridge:
//   - a vertex buffer (4 vertices, each a vec2 position + vec3 color) in host-visible memory;
//   - an index buffer (6 indices = two triangles forming the quad);
//   - a pipeline that declares vertex input (1 binding, 2 attributes);
//   - vkCmdBindVertexBuffers / vkCmdBindIndexBuffer / vkCmdDrawIndexed at draw time.
// A push-constant angle rotates the quad in 2D so it spins (frame-based, like the other samples). The
// quad is 2D and faces the camera, so no depth attachment is needed. The same binary runs through the
// shim (vulkan-shim.dll) and natively against a real driver (pass vulkan-1.dll), validating fidelity.
...
#include "vertex_buffer_spirv.hxx"
...
namespace
{
    ...
    // Rotation is driven by real wall-clock time (radians/second), not the frame count, so the quad spins
    // at the same physical rate natively and emulated (a frame-count-based angle would spin faster at a
    // higher frame rate, making a host-vs-emulated comparison misleading).
    constexpr float rotation_speed = 0.9f;

    // One vertex: a 2D position and an RGB color. Matches the pipeline's vertex input below.
    struct Vertex
    {
    ...
    // A quad (centered, half-extent 0.5) with a distinct color per corner.
    ...
    // Wall-clock milliseconds from the system time (GetTickCount64 runs far too fast in the emulator).
    uint64_t now_ms()
    {
        FILETIME ft{};
...
int main(int argc, char** argv)
{
    ...
    wc.lpszClassName = "SogenVulkanVertexBuffer";
    wc.hCursor = LoadCursorA(nullptr, IDC_ARROW);
    if (!RegisterClassExA(&wc))
    {
        std::printf("[vk-vb] RegisterClassExA failed: %lu\n", GetLastError());
        return 1;
    }

    // Fixed-size window so the surface extent never changes (this sample does not recreate the swapchain).
    constexpr DWORD window_style = WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX | WS_VISIBLE;
    const HWND hwnd =
        CreateWindowExA(0, wc.lpszClassName, "Sogen Vulkan - Vertex Buffer", window_style, 200, 200, static_cast<int>(window_width),
                        static_cast<int>(window_height), nullptr, nullptr, hinstance, nullptr);
    if (!hwnd)
    {
        std::printf("[vk-vb] CreateWindowExA failed: %lu\n", GetLastError());
        return 2;
    }
    ...
    if (!mod)
    {
        std::printf("[vk-vb] LoadLibrary(%s) failed: %lu\n", dll, GetLastError());
        return 3;
    }
    ...
    if (!gipa)
    {
        std::printf("[vk-vb] no vkGetInstanceProcAddr export\n");
        return 4;
    }
    ...
    {
        std::printf("[vk-vb] vkCreateInstance failed (no host Vulkan driver?)\n");
        return 5;
    }
    ...
    if (device_count == 0)
    {
        std::printf("[vk-vb] no physical devices\n");
        return 6;
    }
    ...
    {
        std::printf("[vk-vb] no graphics queue family\n");
        return 7;
    }
    ...
    {
        std::printf("[vk-vb] vkCreateDevice failed\n");
        return 8;
    }
    ...
    // --- vertex + index buffers (host-visible, filled via map/unmap) ---
    VkPhysicalDeviceMemoryProperties mem_props{};
    get_memory_properties(physical_device, &mem_props);
    // Pick a host-visible + host-coherent memory type so the mapped write is visible without flushing.
    ...
    // Creates a host-visible buffer of `size` with `usage`, binds memory, and copies `src` into it.
    ...
    {
        std::printf("[vk-vb] failed to create vertex/index buffers\n");
        return 9;
    }
    ...
    {
        std::printf("[vk-vb] vkCreateWin32SurfaceKHR failed\n");
        return 10;
    }

    // Size the swapchain to the surface's real extent; fall back to the client rect if the bridge reports
    // an undefined extent (0xFFFFFFFF).
    ...
    swci.presentMode = VK_PRESENT_MODE_IMMEDIATE_KHR; // no vsync: FPS reflects real throughput
    swci.clipped = VK_TRUE;

    VkSwapchainKHR swapchain = VK_NULL_HANDLE;
    if (create_swapchain(device, &swci, nullptr, &swapchain) != VK_SUCCESS || !swapchain)
    {
        std::printf("[vk-vb] vkCreateSwapchainKHR failed\n");
        return 11;
    }
    ...
    {
        std::printf("[vk-vb] vkCreateRenderPass failed\n");
        return 12;
    }
    ...
    vert_ci.codeSize = vertex_buffer_vert_spv.size() * sizeof(uint32_t);
    vert_ci.pCode = vertex_buffer_vert_spv.data();
    ...
    frag_ci.codeSize = vertex_buffer_frag_spv.size() * sizeof(uint32_t);
    frag_ci.pCode = vertex_buffer_frag_spv.data();
    ...
    // Vertex input: one interleaved binding, two attributes (vec2 position, vec3 color).
    ...
    {
        std::printf("[vk-vb] vkCreateGraphicsPipelines failed\n");
        return 13;
    }
    ...
    // --- render loop: spin the quad (frame-based angle) and track FPS (system-time based) ---
    ...
    while (!g_quit && frame < max_frames)
    {
        ...
        {
            std::printf("[vk-vb] vkAcquireNextImageKHR -> %d\n", acquired);
            break;
        }
        ...
        {
            std::printf("[vk-vb] vkQueuePresentKHR -> %d\n", presented);
            break;
        }
        ...
        if (fps_window_frames >= fps_window_size)
        {
            ...
            std::snprintf(title.data(), title.size(), "Sogen Vulkan - Vertex Buffer - %.2f FPS", fps);
            SetWindowTextA(hwnd, title.data());
            std::printf("[vk-vb] %.2f FPS (%.0f ms/frame, frame %u)\n", fps, ms_per_frame, frame);
            fps_window_frames = 0;
    ...
    std::printf("[vk-vb] done after %u frames\n", frame);
