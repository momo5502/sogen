...
static bool open_trace_instruction = false;
...
namespace
{
    ...
    std::string get_instruction_string(const disassembler& d, const emulator& emu, const uint64_t address)
    {
        ...
        uint16_t reg_cs = 0;
        emulator& emu_ref = const_cast<emulator&>(emu);
    ...
    void handle_instruction(analysis_context& c, const uint64_t address)
    {
        ...
        const auto current_ip = c.win_emu->current_thread().current_ip;
        ...
        uint64_t dump_addr_reg = 0;

        if (current_ip == (ULONGLONG)0x4B2E64E0)
        {
            //open_trace_instruction = true;
        }

        //if (current_ip == (ULONGLONG)0x4B2F50D6)
        //{
        //    open_trace_instruction = false;
        //}

        // Print instruction details for every instruction
        if (open_trace_instruction)
        {
            // Find module for current instruction
            const auto* current_module = win_emu.mod_manager.find_by_address(address);
            const char* module_name = current_module ? current_module->name.c_str() : "Unknown";

            // only print Module: ntdll.dll and reg_ss == 0x23

            auto& emu = win_emu.emu();

            // Disassemble current instruction
            std::string instruction_str = "<invalid>";
            constexpr size_t MAX_INSTRUCTION_BYTES = 15;
            std::array<uint8_t, MAX_INSTRUCTION_BYTES> instruction_bytes{0};
            const auto result = emu.try_read_memory(address, instruction_bytes.data(), instruction_bytes.size());
            if (result)
            {
                // Get CS register value to determine 32-bit vs 64-bit mode
                const auto cs = emu.reg<uint16_t>(x86_register::cs);
                const auto instructions = c.d.disassemble(emu, cs, instruction_bytes, 1);
                if (!instructions.empty())
                {
                    const auto& inst = instructions[0];
                    instruction_str = std::string(inst.mnemonic) + (strlen(inst.op_str) ? " " + std::string(inst.op_str) : "");
                }
            }
            ...
            // Read CS segment first to determine register display mode
            const auto reg_cs = emu.reg<uint16_t>(x86_register::cs);
            const auto reg_ss = emu.reg<uint16_t>(x86_register::ss);
            const auto reg_ds = emu.reg<uint16_t>(x86_register::ds);
            const auto reg_es = emu.reg<uint16_t>(x86_register::es);
            const auto reg_fs = emu.reg<uint16_t>(x86_register::fs);
            const auto reg_gs = emu.reg<uint16_t>(x86_register::gs);

            if (reg_cs == 0x33 || reg_cs == 0x08)
            {
                // 64-bit mode - display 64-bit registers
                const auto rax = emu.reg<uint64_t>(x86_register::rax);
                const auto rbx = emu.reg<uint64_t>(x86_register::rbx);
                ...
                const auto rdi = emu.reg<uint64_t>(x86_register::rdi);
                const auto rsp = emu.reg<uint64_t>(x86_register::rsp);
                const auto rbp = emu.reg<uint64_t>(x86_register::rbp);
                const auto r8 = emu.reg<uint64_t>(x86_register::r8);
                const auto r9 = emu.reg<uint64_t>(x86_register::r9);
                const auto r10 = emu.reg<uint64_t>(x86_register::r10);
                const auto r11 = emu.reg<uint64_t>(x86_register::r11);
                const auto r12 = emu.reg<uint64_t>(x86_register::r12);
                const auto r13 = emu.reg<uint64_t>(x86_register::r13);
                const auto r14 = emu.reg<uint64_t>(x86_register::r14);
                const auto r15 = emu.reg<uint64_t>(x86_register::r15);
                const auto rip = emu.reg<uint64_t>(x86_register::rip);
                const auto gs_base = emu.reg<uint64_t>(x86_register::gs_base);
                const auto fs_base = emu.reg<uint64_t>(x86_register::fs_base);
                dump_addr_reg = r15;

                if (0)
                {
                    // Print 64-bit registers in a formatted layout
                    win_emu.log.print(color::dark_gray,
                                      "  RAX: %016llX  RBX: %016llX  RCX: %016llX  RDX: %016llX\n"
                                      "  RSI: %016llX  RDI: %016llX  RSP: %016llX  RBP: %016llX\n"
                                      "  R8:  %016llX  R9:  %016llX  R10: %016llX  R11: %016llX\n"
                                      "  R12: %016llX  R13: %016llX  R14: %016llX  R15: %016llX\n"
                                      "  RIP: %016llX  GS_BASE: %016llX  FS_BASE: %016llX\n"
                                      "  Segments - CS: 0x%04x  SS: 0x%04x  DS: 0x%04x  ES: 0x%04x  FS: 0x%04x  GS: 0x%04x\n",
                                      rax, rbx, rcx, rdx, rsi, rdi, rsp, rbp, r8, r9, r10, r11, r12, r13, r14, r15, rip, gs_base, fs_base,
                                      reg_cs, reg_ss, reg_ds, reg_es, reg_fs, reg_gs);
                }
            }
            else if (reg_cs == 0x23 || reg_cs == 0x18)
            {
                // 32-bit mode - display 32-bit registers
                ...
                const auto eip = emu.reg<uint64_t>(x86_register::eip);
                const auto gs_base = emu.reg<uint64_t>(x86_register::gs_base);
                const auto fs_base = emu.reg<uint64_t>(x86_register::fs_base);
                // Use the full stack pointer for memory operations, not the truncated ESP
                dump_addr_reg = eax;

                if (strstr(module_name, "ntdll") != 0 && reg_cs == 0x23)
                {
                    // Print 32-bit registers in a formatted layout
                    win_emu.log.print(color::dark_gray,
                                      "  EAX: %08X      EBX: %08X      ECX: %08X      EDX: %08X\n"
                                      "  ESI: %08X      EDI: %08X      ESP: %08X      EBP: %08X\n"
                                      "  EIP: %016llX  GS_BASE: %016llX  FS_BASE: %016llX\n"
                                      "  Segments - CS: 0x%04x  SS: 0x%04x  DS: 0x%04x  ES: 0x%04x  FS: 0x%04x  GS: 0x%04x\n",
                                      eax, ebx, ecx, edx, esi, edi, esp, ebp, eip, gs_base, fs_base, reg_cs, reg_ss, reg_ds, reg_es, reg_fs,
                                      reg_gs);
                }
            }
            else
            {
                // Unknown CS value - display basic info
                dump_addr_reg = emu.reg<uint64_t>(x86_register::rsp);
                win_emu.log.print(color::dark_gray,
                                  "  Unknown CS segment: 0x%04x\n"
                                  "  Segments - CS: 0x%04x  SS: 0x%04x  DS: 0x%04x  ES: 0x%04x  FS: 0x%04x  GS: 0x%04x\n",
                                  reg_cs, reg_cs, reg_ss, reg_ds, reg_es, reg_fs, reg_gs);
            }
#endif // 0
            ...
            if (strstr(module_name, "ntdll") != 0 && reg_cs == 0x23)
            {
                ...
                win_emu.log.print(color::gray, "Module: %s | %016llX | %-15s | via: %016llX\n", module_name, address,
                                  instruction_str.c_str(), previous_ip);
            }

            INT AAADEBUG = 0;
        }

        if (open_trace_instruction && 0) // open_trace_instruction
        {
            // Print memory dump in db format (like windbg)
            uint64_t dump_addr = 0x000000000009fd20;
            constexpr size_t dump_size = 0x300;
            std::vector<uint8_t> memory_data(dump_size);

            auto& emu = win_emu.emu();
            if (emu.try_read_memory(dump_addr, memory_data.data(), dump_size))
            {
                win_emu.log.print(color::cyan, "Memory dump at 0x%llX (db format):\n", dump_addr);

                // Print in db format (16 bytes per line)
                for (size_t i = 0; i < dump_size; i += 16)
                {
                    win_emu.log.print(color::cyan, "%016llX: ", dump_addr + i);
                    ...
                    for (size_t j = 0; j < 16; ++j)
                    {
                        if (i + j < dump_size)
                        {
                            win_emu.log.print(color::cyan, "%02X ", memory_data[i + j]);
                        }
                        else
                        {
                            win_emu.log.print(color::cyan, "   "); // padding for incomplete lines
                        }
                    ...
                    {
                        uint8_t byte = memory_data[i + j];
                        char ascii_char = (byte >= 32 && byte <= 126) ? static_cast<char>(byte) : '.';
                        win_emu.log.print(color::cyan, "%c", ascii_char);
                    }
            ...
            {
                win_emu.log.print(color::red, "Failed to read memory at 0x%llX\n", dump_addr);
            }
