...
namespace sogen
{
    ...
    class guest_object
    {
      public:
        ...
        guest_object(memory_interface& memory, const uint64_t address = 0)
            : memory_(&memory),
              address_(address)
        {
        }

        guest_object() = default;
        ...
        guest_object<T> shift(const int64_t offset) const
        {
            return guest_object<T>(*this->memory_, this->address_ + offset);
        }
    ...
    class guest_allocator
    {
      public:
        guest_allocator() = default;

        guest_allocator(memory_interface& memory)
            : memory_(&memory)
        {
        }

        guest_allocator(memory_interface& memory, const uint64_t address, const uint64_t size)
            : memory_(&memory),
              address_(address),
              size_(size),
        ...
        template <typename T>
        guest_object<T> reserve(const size_t count = 1)
        {
            const auto potential_start = this->reserve(sizeof(T) * count, alignof(T));
            return guest_object<T>(*this->memory_, potential_start);
        }
    ...
    std::basic_string<Element> read_guest_string(memory_interface& mem, const uint64_t address, const std::optional<size_t> size = {})
    {
