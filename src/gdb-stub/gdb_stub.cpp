...
namespace gdb_stub
{
    namespace
    {
        ...
        void send_file_result(const debugging_context& c, unsigned result)
        {
            c.connection.send_reply("F" + utils::string::to_hex_number(result));
        }

        void send_file_error(const debugging_context& c, unsigned error)
        {
        ...
        void process_file_write(const debugging_context& c, const std::string_view payload)
        {
            ...
            send_file_result(c, static_cast<int>(n));
        }
