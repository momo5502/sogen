...
self.addEventListener('activate', (event) => {
  event.waitUntil((async () => {
    for (const name of await caches.keys()) {
      if (name.startsWith('sogen-macos-root-') && name !== CACHE) await caches.delete(name);
    }
...
function blockRequest(identity, index) {
  return new Request(`${self.location.origin}/__sogen-block/${encodeURIComponent(blockKey(identity))}/${index}`);
}
...
async function identify(url, cache) {
  ...
  try {
    ...
    if (head.ok) {
      identity = {
        ...
        etag: head.headers.get('ETag') || head.headers.get('Last-Modified') || 'none',
        size: Number(head.headers.get('Content-Length')) || 0,
        type: head.headers.get('Content-Type') || 'application/octet-stream',
      };
    }
  } catch { /* offline: a recorded identity is better than refusing to serve */ }
  ...
  if (!previous || previous.etag !== identity.etag || previous.size !== identity.size) {
    await cache.put(recordKey, new Response(JSON.stringify(identity), {
...
async function fillBlocks(url, identity, first, last, cache) {
  ...
  while (index <= last) {
    let end = index;
    while (end + 1 <= last && !(await cache.match(blockRequest(identity, end + 1)))) ++end;
    ...
    const response = await fetch(url, { headers: { Range: `bytes=${from}-${to}` }, cache: 'no-store' });
    ...
    for (let block = index; block <= end; ++block) {
      ...
      if (slice.length !== BLOCK && blockStart + slice.length !== total) continue;

      await cache.put(blockRequest(identity, block), new Response(slice, {
        headers: {
...
async function loadBlocks(url, identity, first, last, cache) {
  ...
  if (missingFrom >= 0) {
    ...
    for (let index = missingFrom; index <= missingTo; ++index) {
      if (!blocks[index - first]) blocks[index - first] = await cache.match(blockRequest(identity, index));
    }
  }
...
async function serveRange(request, url, header) {
  ...
  if (match[1] === '') {
    ...
    end = match[2] === '' ? identity.size - 1 : Math.min(Number(match[2]), identity.size - 1);
  }
...
self.addEventListener('fetch', (event) => {
  ...
  if (url.pathname.startsWith('/__sogen-block/') || url.searchParams.has('__sogen_identity')) return;

  const header = request.headers.get('Range');
  if (!header) return;

  event.respondWith(serveRange(request, url.origin + url.pathname, header).catch(() => fetch(request)));
});
...
self.addEventListener('message', (event) => {
  ...
  if (data.t === 'sogen-cache-status') {
    event.waitUntil((async () => {
      ...
      reply({ t: 'status', blocks, bytes: blocks * BLOCK, block: BLOCK, files: versions.size });
    })());
    return;
  }
    ...
    event.waitUntil((async () => {
      ...
      try {
        ...
        if (!identity || !identity.size) {
          reply({ t: 'warmed', url: data.url, done: 0, total: 0, error: 'no identity' });
          return;
        }

        const total = Math.ceil(identity.size / BLOCK);
        const from = Math.min(data.from || 0, total);
        const to = Math.min(data.to === undefined ? total - 1 : data.to, total - 1);

        for (let index = from; index <= to; ++index) {
          if (!(await cache.match(blockRequest(identity, index)))) {
            await fillBlocks(data.url, identity, index, Math.min(index + 7, to), cache);
            index = Math.min(index + 7, to);
          }
        }

        reply({ t: 'warmed', url: data.url, done: to - from + 1, total, size: identity.size });
      } catch (error) {
        reply({ t: 'warmed', url: data.url, error: String(error && error.message ? error.message : error) });
      }
    })());
    return;
  }
