    ...
    namespace
    {
        ...
        class sdl_ui_backend final : public ui_backend
        {
          public:
            ...
            explicit sdl_ui_backend(const ui_backend_options& options)
                : raise_new_windows_(options.raise_new_windows)
            {
            }
            ...
            void create_window_impl(const ui_window_desc& desc)
            {
                ...
                // macOS routes pointer and key events only to the frontmost application, and a process
                // that was started from a shell it did not put in front never becomes one. A guest's own
                // -[NSApplication activateIgnoringOtherApps:] cannot fix that: it is answered inside the
                // emulator and never reaches the host. Without this the window opens behind whatever was
                // in front and collects nothing.
                if (this->raise_new_windows_ && (flags & SDL_WINDOW_HIDDEN) == 0)
                {
