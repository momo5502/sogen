...
namespace sogen::test
{
    namespace
    {
        ...
        class fake_host_memory : public memory_interface
        {
          public:
            ...
            // reserved_host_ranges() scan deliberately OMITS - modelling the FEX/Apple wow64 rebase host
            // window [0x400000000, 0x500000000), which the full scan skips (it is guest address space, not a
            // foreign occupant) yet the windowed probe correctly reports occupied for a native pick landing
            // on it. This asymmetry is exactly why find_free_host_allocation_base's rescan must include the
            // windowed reserve_host_memory_ranges_in(pick): a full-only rescan never records these, so a pick
            // that lands here is re-picked identically every iteration until the retry cap.
    ...
    // Regression coverage for the wow64 rebase-window blind spot (real busybox-w32 wow64 init failed here,
    // C0000145 in ~6/8 runs). find_free_host_allocation_base confirms a pick with the windowed
    // host_window_is_free probe but, before the fix, rescanned only via the full reserve_host_memory_ranges().
    // On the FEX/Apple backend the wow64 host window is reported occupied by the windowed probe (for a native
    // pick landing on it) yet omitted from the full scan, so the full-only rescan recorded nothing new and the
    // same occupied pick was chosen every iteration until the retry cap - the allocation failed outright. The
    // rescan now also records the just-rejected pick window via reserve_host_memory_ranges_in, so the next
    // pick steps past a range the full scan cannot see. hidden_from_full_scan models exactly that range.
    ...
    // Regression coverage for the complementary need: the rescan must ALSO keep the full
    // reserve_host_memory_ranges(), not just the windowed record. A large contiguous foreign region at the
    // naive pick - wider than max_host_reserved_retries picks - can only be cleared in the bounded retry
    // budget if the full scan records the whole region at once; recording just one pick-sized slice per
    // iteration would exhaust the retries mid-region and fail (real regression: a relocated 64-bit module,
    // user32.dll, could not be placed when only the windowed slice was recorded). The region here is visible
    // to the full scan and spans far more than the retry budget could step past one slice at a time.
