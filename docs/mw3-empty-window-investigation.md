...
## Update (2026-06-12, BREAKTHROUGH): geometry rasterises; the real fragment shader discards

Two env-gated experiments settle it:

- **`EMULATOR_FORCE_FS=<path-to-constant-colour.spv>`** replaces every pipeline's fragment shader with
  a trivial `out = vec4(1,0,1,1)` module (no discard, no texture). Result: the **entire window is
  magenta** (`present readback 2776812/3702416 nonzero, px0=0xffff00ff`). So the game's geometry
  rasterises perfectly and covers the screen — the black menu is **not** geometry, transform, vertex
  data, swizzle, or texture upload.
- **`EMULATOR_FORCE_ALPHA_PASS`** forces the optimized spec-constant path (`IsOptimized` id =
  `MaxNumSpecConstants`; 12 in DXVK 2.7.x, 20 in newer) with `AlphaCompareOp = ALWAYS`. With a green
  clear the readback becomes **black, not green** — i.e. fragments now *write* (overwriting the clear),
  proving the alpha-test discard was the blocker. They write black because forcing the optimized path
  also zeroes the fixed-function texture-stage spec constants (an artifact of the override).

Conclusion: the single root cause is the **empty SpecData UBO**. The fragment shader reads
`SpecAlphaCompareOp` (and FF stage ops, sampler types) from it (`IsOptimized` defaults false). It is
all-zero in the bridge, so the alpha test is `NEVER` and every fragment is discarded. Populating that
UBO correctly fixes the alpha test, the fixed-function colour path, and sampler types at once.

The diagnostic SPIR-V was hand-assembled (no SPIR-V compiler on the box) — a 70-word constant-colour
fragment shader.

### Concrete fix target

The descriptor for the menu set's **binding 1** (the fixed SpecData/PS constant buffer) references
`mem M @ 0x400000` which reads zero, while DXVK's dense constant data lives in other host-visible
allocations. The streaming VS-constant buffer (binding 0) is correct. So a fixed (non-streaming,
